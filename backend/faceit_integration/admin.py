...
from .models import PlayerFaceitStats, TeamFaceitMatch, PlayerMatchStats, FaceitSyncRun
...
class PlayerMatchStatsAdmin(admin.ModelAdmin):
    list_display = ('player', 'match', 'result', 'kills', 'deaths', 'assists', 'kd_ratio', 'enemies_flashed', 'last_synced_at')
    list_filter = ('result',)
    search_fields = ('player__ingame_name', 'match__faceit_match_id')
    readonly_fields = ('raw_data', 'last_synced_at')
...
class FaceitSyncRunAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'player_match_stats_failed', 'error',
    )
    list_filter = ('trigger',)
