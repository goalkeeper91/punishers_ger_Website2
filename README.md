...
| Teams | `/admin/teams`, `/new`, `/:id/edit` | Team-CRUD, Teambild-Upload, Roster-Verwaltung, manuelle Match-Erfassung, FACEIT-Team-ID je Liga (siehe unten) | Anlegen/Löschen/alle Teams bearbeiten: Admin oder `teams.manage_teams`-Permission; Bearbeiten & Roster & Matches & FACEIT-Registrierung des **eigenen** Teams: zusätzlich Teammanager |
| Ligen | `/admin/leagues` | Liga-CRUD inkl. FACEIT-Organizer-ID (Voraussetzung für den automatischen Match-Abgleich) | Admin oder `teams.manage_teams` – **nicht** für einen einzelnen Teammanager, da eine Liga mehrere Teams betrifft |
...
| Social-Media-Posts | `/admin/social-posts` | Auto-generierte Match-Ankündigungen/-Ergebnisse für Facebook/Instagram/X einsehen, neu generieren, löschen (siehe unten) | `social_posts.manage_social_posts`-Permission (oder Admin) |
...
- **Woher kommen die IDs?** `Player.faceit_player_id` identifiziert den Spieler auf FACEIT (weiterhin nur über Django-Admin befüllbar). `League.faceit_organizer_id` ist die FACEIT-**Organizer**-ID der Liga (ablesbar aus der URL `faceit.com/de/organizers/<id>/<name>`, z.B. getrennt für DACH CS und ESEA) – pflegbar unter `/admin/leagues` (Admin oder `teams.manage_teams`, org-weit, nicht Teammanager-scoped, da eine Liga mehrere Teams betrifft). Da ein Team in mehreren Ligen mit unterschiedlichen FACEIT-Team-IDs registriert sein kann, liegt die FACEIT-Team-ID nicht direkt auf `Team`, sondern auf `TeamLeagueEntry` (Team ↔ League, je Liga eine eigene `faceit_team_id`) – pflegbar im "FACEIT-Registrierung"-Abschnitt auf `/admin/teams/:id/edit` (auch für den Teammanager des eigenen Teams). Fehlt die Organizer-ID an der Liga, wird sie beim Sync **stillschweigend übersprungen** (kein Fehler, s. `sync_all_team_matches`); fehlt/stimmt die Team-ID nicht, findet der Sync einfach keine passenden Matches – beides sieht in `FaceitSyncRun` wie ein normaler Lauf ohne Fehler aus, nur eben mit `matches_created: 0`. Zum Entfernen einer Registrierung das Feld leer speichern statt den Eintrag zu löschen – Löschen würde alle darunter gespeicherten Matches (inkl. manuell erfasster) mitreißen.
- **Wie werden Matches gefunden?** Pro Liga: Organizer-ID → alle Championships/Seasons des Organizers (`GET /organizers/{id}/championships`, **neueste zuerst** via `sort=-createdAt` – siehe `FaceitClient.get_organizer_championships` –, paginiert bis `MAX_CHAMPIONSHIP_PAGES` = 20 Seiten à 100 = 2000) → Matches jeder Season (`GET /championships/{id}/matches`, **alle drei** von FACEIT unterstützten `type`-Werte: `upcoming`, `ongoing`, `past`) → gefiltert auf Matches, in denen eine unserer registrierten `faceit_team_id` je Liga vorkommt. So werden automatisch auch neue Seasons erfasst, ohne dass eine Championship-ID von Hand nachgetragen werden muss. Zwei live bestätigte Bugs hier behoben: (1) `ongoing` fehlte ursprünglich (nur `upcoming`+`past` abgefragt) – FACEIT verschiebt ein Match aber schon beim Check-in/Map-Veto dorthin, lange bevor `upcoming` es noch listen würde. (2) Ohne expliziten `sort` ist die Championship-Liste eines Organizers **nicht zuverlässig nach Aktualität geordnet** – bei einem Organizer mit vielen parallelen Divisionen/Cups (hier: DACH CS) konnte eine bereits laufende, brandneue Championship dadurch irgendwo in der (paginierten) Liste stehen und nie erreicht werden, unabhängig vom Cap. Mit `sort=-createdAt` ist der Cap jetzt nur noch ein Tiefen-Limit für die Historie, kein Korrektheitsrisiko mehr – `_get_league_championships` loggt trotzdem eine Warnung, falls er erreicht wird (dann fehlen ggf. die ältesten Championships, nicht die aktuellen). (3) `publishedOnly` steht bei FACEIT standardmäßig auf `true` und wurde nie überschrieben – eine reale, aktuell laufende Championship einer Drittliga (bestätigt korrekte Organizer-ID, `sort`-Fix bereits vorhanden, weit innerhalb des Caps) wurde trotzdem nie zurückgeliefert, offenbar weil sie FACEIT-intern nicht als "published" markiert ist. `get_organizer_championships` setzt `publishedOnly` jetzt explizit auf `false`.
...
- **Manuell:** `POST /admin/faceit/sync/` 🔒👑 legt sofort eine `FaceitSyncRun`-Zeile an, gibt direkt `{started: true, run_id}` zurück und lässt den eigentlichen Sync als FastAPI-`BackgroundTask` weiterlaufen – bewusst **nicht** synchron: bei einem Organizer mit langer Championship-Historie kann das Abklappern aller Seasons/Matches locker den Timeout eines Reverse-Proxys (z.B. nginx' ~60s) überschreiten, was den Aufruf sonst mit einem 504 quittiert, obwohl der Sync serverseitig trotzdem sauber durchläuft. Ergebnis abfragen: `GET /admin/faceit/status/` 🔒👑 zeigt den letzten Lauf (`finished_at` noch `null` = läuft noch). Alternativ `python manage.py sync_faceit` auf der Kommandozeile (läuft synchron bis zum Ende, z.B. für eigenen Cron/Task-Scheduler) – nutzt intern dieselbe Sync-Logik wie der automatische Scheduler.
...
**Manuell erfasste Matches (`/admin/teams/:id/edit`).** `League`/`TeamLeagueEntry` waren bisher nur über das (in dieser Deployment unerreichbare) Django-Admin pflegbar – ein Team ohne dort hinterlegte FACEIT-IDs bekam also nie synchronisierte Matches, und es gab keine Möglichkeit, ein Ergebnis von Hand nachzutragen (z.B. einen Scrim oder ein Match, das FACEIT nie hatte). Auf der Team-Bearbeiten-Seite gibt es jetzt einen "Matches"-Abschnitt (Admin oder Teammanager des eigenen Teams, `teams.manage_teams` – dieselbe Berechtigung wie Roster-Verwaltung, keine eigene): Liga per Dropdown wählen (inkl. der fest angelegten Standard-Liga **"Pracc"**, seit Migration `leagues/migrations/0004_seed_pracc_league.py` in jeder Umgebung automatisch vorhanden – Teams müssen also nicht erst eine echte Liga registriert haben, um ein Ergebnis eintragen zu können), Gegner, Datum, Format (Bo1/Bo2/Bo3/Bo5 – bestimmt nur, wie viele leere Map-Zeilen die Auswahl vorausfüllt, Zeilen lassen sich danach frei hinzufügen/entfernen, z.B. für ein 2:0 beendetes Bo3) und je Map Name + Score angeben; Sieg/Niederlage pro Map wird serverseitig aus den beiden Scores abgeleitet, nicht separat abgefragt.

Backend (`fastapi_app/main.py: create_manual_match`) legt bei Bedarf automatisch die passende `TeamLeagueEntry` an und erzeugt **eine `TeamFaceitMatch`-Zeile pro Map** (nicht eine pro Match) – bei mehr als einer Map teilen sich alle Zeilen einen `series_id`-Wert (`faceit_integration.models.TeamFaceitMatch.series_id`), rein zum Zurück-Gruppieren in der Anzeige; ein Bo1 bleibt wie zuvor eine einzelne Zeile ohne `series_id`. Jede Zeile bekommt eine synthetische `faceit_match_id` (`manual-<uuid>` bzw. `manual-<uuid>-map<n>`), da dieses Feld pflicht+unique ist, aber für eine manuell erfasste Partie keine echte FACEIT-ID existiert – dadurch fasst der echte FACEIT-Sync diese Zeilen nie an. `GET /admin/teams/{id}/matches/` gruppiert beim Auslesen wieder per `series_id` zu einer Karte mit Maps-Liste + Aggregat (`team_maps_won`/`opponent_maps_won`, z.B. "2:1"); Löschen entfernt immer die komplette Serie, nie nur eine einzelne Map. Da jede Map als eigene `status=finished`-Zeile gespeichert wird, tauchen manuelle Matches automatisch **pro Map** im Stats-Dashboard (Map-Win/Loss-Auswertung) und als eine Karte im Match-Highlight-Widget der Startseite auf, ganz ohne zusätzlichen Code – beide lesen ohnehin einfach jede `TeamFaceitMatch`-Zeile bzw. gruppieren clientseitig nicht weiter.

### Match-Highlight-Widget (Startseite)

`GET /matches/highlights/` (öffentlich) liefert eine flache Liste: **ein** "nächstes Match" und **ein** "letztes Match" pro Team, das synchronisierte FACEIT-Match-Daten hat (leer, wenn ein Team noch keine Matches hat). "Nächstes Match" zählt sowohl `status=upcoming` als auch `status=ongoing` – FACEIT verschiebt ein Match schon beim Check-in/Map-Veto in seinen `ongoing`-Bucket, deutlich bevor es wirklich losgeht (siehe FACEIT-Integration oben), das darf aus Besuchersicht aber nicht einfach aus der Anzeige verschwinden, bevor es tatsächlich `finished` ist. Im Frontend zeigt `MatchHighlightWidget` das als schwebende Karte unten rechts auf der Startseite – bewusst kein voller Abschnitt wie bei großen Orgas (NAVI, G2 – siehe Recherche), sondern am "Score Bug"-Muster aus Sport-Streaming-UIs orientiert: kompakt, auffällig durch Slide-in-Animation und Akzentfarbe, aber minimal-invasiv. Rotiert alle 6s durch die komplette Liste, lässt sich minimieren (Pill-Button) oder für die Session komplett schließen (`sessionStorage`).
...
### Social-Media-Post-Entwürfe (`backend/social_posts/`)

Erzeugt automatisch einen Entwurf (Text je Plattform + ein Bild) für Facebook/Instagram/X, sobald ein Match neu als `upcoming`/`ongoing` synct (Ankündigung) oder ein Ergebnis feststeht (Ergebnis-Post) – sowohl über den FACEIT-Sync (`faceit_integration/sync.py: _generate_social_post_draft`, an derselben "nur beim echten Übergang, nicht bei jedem Re-Sync"-Stelle wie die bestehende Discord-Ankündigung) als auch über die manuelle Match-Erfassung (`create_manual_match`, als `BackgroundTask` – LLM+Bild-Rendering soll die "Match hinzugefügt"-Antwort nicht verzögern). Sichtbar unter `/admin/social-posts` (Admin oder `social_posts.manage_social_posts`): pro Entwurf ein Bild-Download-Link, die drei Plattform-Texte je mit Kopieren-Button, sowie "Neu generieren"/"Löschen".

- **Text:** über ein **selbstgehostetes Ollama** (kein externer API-Key, keine laufenden Kosten) – `OLLAMA_BASE_URL` zeigt auf den Container im selben Docker-Netzwerk (z.B. `http://goalkeeper_ollama_prod:11434`), `OLLAMA_MODEL` das gepullte Modell (Standard `llama3.2:3b` – muss exakt dem Namen aus `ollama list` im Ollama-Container entsprechen, inkl. Tag wie `:3b`, sonst 404 "model not found"). Drei getrennte Prompts/Aufrufe statt einem kombinierten – jede Antwort ist einfacher Fließtext statt einem Format, das geparst werden müsste (und dabei leise falsch geparst werden könnte). Leer gelassen (`OLLAMA_BASE_URL`) → Text-Generierung wird übersprungen, das Bild entsteht trotzdem; ein teilweiser Fehlschlag wird als `generation_error` auf dem Entwurf sichtbar, nicht stillschweigend verschluckt.
- **Bild:** bewusst ein **programmatisches Template** (Pillow, `social_posts/image_generation.py`), keine KI-Bildgenerierung – Team-Namen/Score/Liga/Datum müssen exakt stimmen, das ist mit echten Werten in einer festen Vorlage zuverlässiger als mit einem Bildmodell zu hoffen. 1080×1080 (der sicherste, plattformübergreifend kompatible Wert), radialer Verlauf im Marken-Look, ein sehr dezentes Punishers-Logo-Wasserzeichen über die gesamte Fläche als Hintergrund-Textur, dazu eine kleine spielspezifische Badge-Pille oben rechts (z.B. "CS2", "VAL") sowie eine spielspezifische Akzentfarbe (Diagonalen, Banner hinter dem Kicker-Text, Score) statt überall demselben Rot – erkannt per Substring-Match gegen `Team.game` (`GAME_STYLES` in `image_generation.py`, unbekannte Spiele fallen auf das Standard-Rot zurück). Team- und Gegner-Logo als runde Badges links/rechts von Score bzw. "VS" (eigenes Team: lokales `Team.image`, sonst das Punishers-Logo als Fallback; Gegner: FACEIT-Team-Avatar, nur bei synchronisierten Matches vorhanden, HTTP-Abruf mit 5s-Timeout und stillem Verzicht auf das Badge bei Fehlschlag statt Bildabbruch). Sieg/Niederlage bleiben bewusst fest grün/rot statt der Spielfarbe, damit das bei jedem Spiel eindeutig lesbar bleibt. Automatisches Schriftgrößen-Schrumpfen bei langen Team-/Gegnernamen, damit nichts über den Bildrand hinausläuft. Braucht `fonts-dejavu-core` im Docker-Image (Debians `python:3.12-slim`-Basis bringt von Haus aus keine Fonts mit) – ohne das Paket (z.B. lokal ohne Docker) fällt Pillow automatisch auf eine einfache Bitmap-Schrift zurück statt abzustürzen.
- **Maps-Zeile (Bo2/Bo3/Bo5):** bei einer Serie mit mehreren Maps zeigt das Bild zusätzlich bis zu 5 Kacheln (eine pro Map) mit Map-Name und Einzel-Ergebnis (grün/rot je nach Win/Loss). Für Maps, zu denen ein lokales Bild unter `backend/social_posts/assets/maps/` liegt (`MAP_IMAGE_FILES` in `image_generation.py` – aktuell der komplette aktive CS2-Map-Pool: Mirage, Ancient, Inferno, Dust2, Nuke, Anubis, Overpass, plus Vertigo), erscheint dieses Bild als Schwarz/Weiß-Hintergrund der Kachel, mit der Map-Akzentfarbe eingefärbt; ohne lokales Bild bleibt die Kachel einfarbig. Die strukturierten Maps-Daten (`MatchContext.maps` / `SocialPostDraft.maps`, ein JSON-Feld getrennt von `maps_summary`, das nur für den KI-Text-Prompt gedacht ist) werden bisher ausschließlich bei **manuell erfassten** Mehrfach-Map-Matches befüllt (`create_manual_match`) – bei automatisch von FACEIT synchronisierten Ergebnissen fehlt die Kachel-Zeile noch. Der Serien-Score (`team_maps_won`/`opponent_maps_won`) kommt dabei aber korrekt von FACEIT selbst (`TeamFaceitMatch.team_score`/`.opponent_score`, d.h. `results.score` der FACEIT-API) – vorher stand hier fälschlich fest `1:0`/`0:1` (Annahme: jedes Fixture sei ein Bo1), was für eine echte Mehrfach-Map-Serie falsch war.
- **Datenmodell:** `SocialPostDraft` ist bewusst **kein** FK auf `TeamFaceitMatch` – der komplette Match-Kontext (Gegner, Liga, Datum, Maps-Ergebnis) wird beim Erzeugen auf den Entwurf kopiert, damit ein späteres Bearbeiten/Löschen des zugrundeliegenden Matches einen schon erzeugten Entwurf nicht nachträglich invalidiert oder mitreißt.
