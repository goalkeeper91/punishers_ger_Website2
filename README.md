...
- **Wie werden Matches gefunden?** Pro Liga: Organizer-ID → alle Championships/Seasons des Organizers (`GET /organizers/{id}/championships`, paginiert bis `MAX_CHAMPIONSHIP_PAGES` = 60 Seiten à 50 = 3000) → Matches jeder Season (`GET /championships/{id}/matches`, **alle drei** von FACEIT unterstützten `type`-Werte: `upcoming`, `ongoing`, `past`) → gefiltert auf Matches, in denen eine unserer registrierten `faceit_team_id` je Liga vorkommt. So werden automatisch auch neue Seasons erfasst, ohne dass eine Championship-ID von Hand nachgetragen werden muss. Zwei live bestätigte Bugs hier behoben: (1) `ongoing` fehlte ursprünglich (nur `upcoming`+`past` abgefragt) – FACEIT verschiebt ein Match aber schon beim Check-in/Map-Veto dorthin, lange bevor `upcoming` es noch listen würde. (2) Die Championship-Liste des Organizers ist **nicht zuverlässig nach Aktualität sortiert** – bei einem Organizer mit vielen parallelen Divisionen/Cups (hier: DACH CS) kann eine bereits laufende, brandneue Championship irgendwo in der Liste stehen, nicht zwingend vorn. Der alte Cap von 250 (5 Seiten) hat dadurch ein reales, aktuelles Match komplett übersehen – nicht falsch einsortiert, sondern nie angefragt. `_get_league_championships` loggt jetzt eine Warnung, falls der (jetzt viel höhere) Cap trotzdem erreicht wird.
